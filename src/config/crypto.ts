// secret の salt 付き可逆暗号 (平文保存しないため)。
//
// - 一方向 hash ではなく可逆暗号 (クロール時に元のトークンが必要なため)。
// - scrypt(masterSecret, salt) で鍵導出 → AES-256-GCM。
// - master secret は呼び出し側が渡す (env override / マシン束縛値は store が決める)。
//
// 形式 (EncryptedBlob): すべて base64。salt はレコード毎にランダム。
// 出自: Excubitor src/secrets/crypto.ts と同方式 (移植)。
