...
**opt-in ローカル LLM executor (`@ludiars/canalis/llm`、2026-06-09)**: LLM が避けられない
②の最後の手段として、 OpenAI 互換 `/v1/chat/completions` (Ollama/vLLM/LM Studio/llama.cpp
server、 例 ローカル Gemma 4 12B) を叩く `localOpenAiExecutor` を用意する。

- これは **`commandRunner` と同型の dep-free 注入ヘルパ**: SDK 依存も API キー要求も無い
  (raw fetch + baseUrl のみ)。 よって「共有 lib に LLM 依存を足さない」 に抵触しない。
- **root の公開面 (core/crawl/save) には混ぜず**、 別サブパス `@ludiars/canalis/llm` に隔離。
  core/crawl/save は import も呼出しもしない。 LLM を使うサービス② が明示的に import して
  `LlmExecutor` を DI する (SqlExecutor/CypherExecutor/FtRunner と同じ注入思想)。
- 失敗 (未起動/HTTPエラー/空応答) は throw。 ②側で握って **決定論フォールバック** に倒すこと
  (LLM は従、 heuristic が主)。
